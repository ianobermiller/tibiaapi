...
{
    public class FloorChangeDownPacket : IncomingPacket
    {
        ...
        public FloorChangeDownPacket(Objects.Client c)
            : base(c)
        {
            Type = IncomingPacketType_t.FLOOR_CHANGE_DOWN;
        ...
        public override bool ParseMessage(NetworkMessage msg, PacketDestination_t destination, Objects.Location pos)
        {
            ...
            {
                int j, i;
                for (i = (int)pos.Z, j = -1; i < (int)pos.Z + 3; ++i, --j)
                {
                    if (!setFloorDescription(msg, pos.X - 8, pos.Y - 6, i, 18, 14, j))
                    {
