...
namespace Tibia.Objects
{
    public partial class Client
    {
        public class MemoryHelper
        {
            ...
            public string ReadTextField(long address)
            {
                //DistanceText = 0;
                //DistanceTextLength = 16;
                //DistanceFieldType = 20;
                if (client.Memory.ReadByte(address + 20) > 0xF)
                {
                    return client.Memory.ReadString(client.Memory.ReadUInt32(address));
                }
                else
                {
                    return client.Memory.ReadString(address);
                }
            }
