...
namespace Tibia.Packets
{
    public class Proxy : ProxyBase
    {
        ...
        private void Initialize()
        {
            ...
            //login event
            base.ReceivedInitGameIncomingPacket += new IncomingPacketListener(Proxy_ReceivedSelfAppearIncomingPacket);
            ...
            try
            {
                ...
                switch (protocolId)
                {
                    case OutgoingPacketType.LoginServerRequest:
                        ...
                        clientRecvMsg.GetUInt16();//os
                        clientRecvMsg.GetUInt16();//version
						if(Version.CurrentVersion >=971)
						{
							clientRecvMsg.GetUInt16();//preview?
							clientRecvMsg.GetUInt16();//padding?
						}
                    ...
                    case OutgoingPacketType.EnterGame:
                        ...
                        switch (cmd)
                        {
                            ...
                            case 0x64: //character list
                                ...
                                for (int i = 0; i < nChar; i++)
                                {
                                    ...
                                    if (Version.CurrentVersion >= 971)
                                    {
                                        charList[i].IsPreview = Convert.ToBoolean(serverRecvMsg.GetByte());
                                    }
                                }
                                break;
                            default:
