...
{
    public class RotatePacket : OutgoingPacket
    {
        public Constants.Direction Direction { get; set; }

        public RotatePacket(Objects.Client c, Constants.Direction direction)
            : base(c)
        {
            Direction = direction;
        ...
        {
            RotatePacket p = new RotatePacket(client, direction);
            return p.Send();
        }
    }
}